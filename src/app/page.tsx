    ...
    <div className="min-h-full flex flex-col">
      ...
      <main className="mx-auto w-full max-w-6xl flex-1 px-4 py-8">
        ...
        <div className="rounded-xl border">
          {righe.length === 0 ? (
            <TableEmpty>
              <p className="font-medium text-foreground">Nessuno sku trovato</p>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="text-right">Prezzo eBay</TableHead>
                ...
                {righe.map((r) => (
                  <TableRow key={r.id} className="cursor-pointer">
                    <TableCell className="p-0">
                      <Link href={`/magazzino/${r.id}`} className="block font-mono text-xs p-3">
                        {r.skuCode}
                      </Link>
                    </TableCell>
                    <TableCell className="p-0">
                      <Link href={`/magazzino/${r.id}`} className="block p-3">
                        {r.artista}
                      </Link>
                    </TableCell>
                    <TableCell className="p-0">
                    ...
                    <TableCell className="text-muted-foreground">
                      {formatMisura(r.larghezza, r.altezza)}
                    </TableCell>
                    <TableCell className="text-muted-foreground">{r.tipo}</TableCell>
                    <TableCell>
                      <Badge variant={condizioneVariant(r.condizione)}>{r.condizione}</Badge>
                    </TableCell>
                    <TableCell className="text-right tabular-nums">
                      {r.quantitaDisponibile}
                    </TableCell>
                    <TableCell className="text-right tabular-nums">
                      {r.prezzoEbay ? `€ ${r.prezzoEbay}` : "—"}
                    </TableCell>
                    <TableCell>
                      {r.bloccatoVendita ? (
